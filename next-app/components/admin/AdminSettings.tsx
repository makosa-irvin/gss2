...
export function AdminSettings({ settings, onSave, onChangePassword }: { settings: CompanySettings; onSave: (settings: CompanySettings) => Promise<void>; onChangePassword: (currentPassword: string, newPassword: string) => Promise<void> }) {
  ...
  return <div className="space-y-6 max-w-4xl">
    <form onSubmit={submit} className="space-y-6">
      <div><div className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-[#9e7120]"><Settings className="w-4 h-4" /><span>Configuration</span></div><h1 className="font-serif-luxury text-3xl font-bold text-[#161f19] mt-1">Company Settings</h1><p className="text-sm text-[#707f74] mt-1">Changes take effect across the whole website and WhatsApp links as soon as you save. Homepage and About page copy now live under their own sections in the sidebar.</p></div>
