        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 flex items-center justify-between gap-4 overflow-visible">
          ...
          <div className="hidden lg:flex items-center gap-0.5 overflow-visible">
            ...
            <div className="relative z-[120]" onMouseEnter={() => setActiveDropdown('safaris')} onMouseLeave={() => setActiveDropdown(null)}>
              ...
              {activeDropdown === 'safaris' && <div className="absolute top-full left-0 z-[200] w-72 rounded-2xl bg-white border border-border-strong shadow-2xl p-2.5 grid grid-cols-1 gap-0.5"><Link onClick={closeMenus} href="/safaris" className="w-full text-left px-3 py-2.5 rounded-xl text-sm font-bold text-brand-deep bg-surface-muted hover:bg-surface-soft">All Safaris & Packages</Link>{['Big 5','Great Migration','Family','Honeymoon','Senior Friendly','Luxury','Midrange','Budget','Fly-In','Safari & Beach'].map(style => <Link key={style} onClick={closeMenus} href={`/safaris?travelStyle=${encodeURIComponent(style)}`} className={dropdownItemClass}>{style} Safaris</Link>)}</div>}
            </div>
            <div className="relative z-[120]" onMouseEnter={() => setActiveDropdown('holidays')} onMouseLeave={() => setActiveDropdown(null)}>
              ...
              {activeDropdown === 'holidays' && <div className="absolute top-full left-0 z-[200] w-64 rounded-2xl bg-white border border-border-strong shadow-2xl p-2.5"><Link onClick={() => { setIsKenyanResidentMode(true); setActiveCurrency('KES'); window.localStorage.setItem(RESIDENT_KEY, 'true'); window.localStorage.setItem(CURRENCY_KEY, 'KES'); closeMenus(); }} href="/hotels?resident=true" className="w-full text-left px-3 py-2.5 rounded-xl text-sm font-bold text-action bg-action-soft hover:bg-surface-soft mb-1 block">Kenyan Resident Beach Deals</Link><Link onClick={closeMenus} href="/hotels" className={dropdownItemClass}>All Beach Resorts & Spas</Link><Link onClick={closeMenus} href="/safaris?travelStyle=Safari%20%26%20Beach" className={dropdownItemClass}>Bush & Beach Combos</Link></div>}
            </div>
