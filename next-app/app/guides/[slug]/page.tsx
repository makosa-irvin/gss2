...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!guide) return {};
  return { title: guide.title, description: guide.description, alternates: { canonical: `/guides/${guide.slug}` }, openGraph: { title: guide.title, description: guide.description, type: 'article', url: `/guides/${guide.slug}`, images: [guide.image] } };
}
...
export default async function GuideDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="max-w-5xl mx-auto px-4 sm:px-8 py-10 sm:py-14 space-y-8">
    ...
    {guide.related?.length ? <nav aria-label="Related safari planning" className="grid sm:grid-cols-2 gap-3">{guide.related.map(item => <Link key={item.to} href={item.to} className="min-h-14 rounded-2xl border border-white/10 bg-white/5 px-5 inline-flex items-center justify-between gap-3 text-sm font-bold text-white hover:border-brand-strong hover:bg-white/[0.07]">{item.label}<ArrowRight className="w-4 h-4 text-brand-soft" /></Link>)}</nav> : null}
    <section className="rounded-3xl bg-shell border border-white/10 p-7 sm:p-9"><h2 className="font-serif-luxury text-3xl font-bold text-white">Ready to turn the research into a route?</h2><p className="text-sm text-on-shell-muted mt-2">Use the safari builder or compare itinerary ideas, then send the options you like in one quote basket.</p><div className="mt-5 flex flex-wrap gap-3"><Link href="/safari-builder" className="min-h-12 px-6 rounded-xl bg-brand-soft text-ink-strong font-extrabold text-sm inline-flex items-center">Build my safari</Link><Link href="/safaris" className="min-h-12 px-6 rounded-xl border border-white/20 text-white font-bold text-sm inline-flex items-center">Explore safari ideas</Link></div></section>
  </div>;
}
