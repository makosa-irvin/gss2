...
import { ArrowRight, Camera, Car, Compass, MessageCircle, ShieldCheck } from 'lucide-react';
import { EnquiryButton } from '../../components/EnquiryButton';
import { getSettings } from '../../lib/api';
import { DEFAULT_ABOUT } from '../../lib/cmsDefaults';

export const metadata: Metadata = { title: 'About Good Secrets Safaris', description: 'Meet the East Africa-based team and planning approach behind Good Secrets Safaris: private itineraries, local support, experienced guides and direct trip planning.', alternates: { canonical: '/about' } };
...
export default async function AboutPage() {
  ...
  return <div className="pb-20">
    <section className="relative min-h-[560px] overflow-hidden border-b border-white/10">
      <Image src={about.teamPhoto || DEFAULT_ABOUT.teamPhoto} alt="The Good Secrets Safaris team in East Africa" fill priority sizes="100vw" className="object-cover" />
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-8 py-16 sm:py-24 min-h-[560px] flex items-end">
        <div className="max-w-3xl space-y-5">
          <span className="inline-flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-brand-soft"><Compass className="w-4 h-4" />{about.eyebrow}</span>
          <h1 className="font-serif-luxury text-4xl sm:text-6xl lg:text-7xl font-bold text-white leading-[1.05]">{about.title}</h1>
          <p className="text-base sm:text-xl text-white/90 leading-relaxed max-w-2xl">{about.intro}</p>
          <div className="flex flex-col sm:flex-row gap-3 pt-2"><EnquiryButton label="Talk to our safari team" className="min-h-12 px-7 rounded-xl bg-brand-soft hover:bg-brand-soft text-ink-strong font-extrabold text-sm shadow-lg" /><Link href="/reviews" className="min-h-12 px-7 rounded-xl border border-white/30 bg-black/20 hover:bg-black/35 text-white font-bold text-sm inline-flex items-center justify-center gap-2">Read independent reviews <ArrowRight className="w-4 h-4" /></Link></div>
        </div>
      </div>
    </section>
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-8 py-14 sm:py-20 space-y-20">
      <section className="grid lg:grid-cols-12 gap-10 lg:gap-14 items-center">
        <div className="lg:col-span-5 space-y-5"><span className="text-xs font-bold uppercase tracking-widest text-brand-soft">Why direct matters</span><h2 className="font-serif-luxury text-3xl sm:text-4xl font-bold text-white">{about.storyTitle}</h2>{about.storyParagraphs.map((paragraph, index) => <p key={index} className="text-base text-on-shell-muted leading-relaxed">{paragraph}</p>)}</div>
        <div className="lg:col-span-7 grid grid-cols-2 gap-4"><div className="col-span-2 sm:col-span-1 rounded-3xl overflow-hidden aspect-[4/5] border border-white/10 relative"><Image src="/images/catalog/family-safari-game-drive.jpg" alt="Family enjoying a safari game drive" fill sizes="(min-width: 1024px) 29vw, (min-width: 640px) 50vw, 100vw" className="object-cover" /></div><div className="col-span-2 sm:col-span-1 grid gap-4"><div className="rounded-3xl overflow-hidden aspect-[16/10] border border-white/10 relative"><Image src="/images/catalog/picnic-lunch-in-the-wild.jpg" alt="Safari picnic lunch in the wild" fill sizes="(min-width: 1024px) 29vw, (min-width: 640px) 50vw, 100vw" className="object-cover" /></div><div className="rounded-3xl overflow-hidden aspect-[16/10] border border-white/10 relative"><Image src="/images/catalog/honey-moon-in-samburu.jpg" alt="Couple on a Samburu safari" fill sizes="(min-width: 1024px) 29vw, (min-width: 640px) 50vw, 100vw" className="object-cover" /></div></div></div>
      </section>
      ...
      <section className="space-y-8">
        ...
        <div className="rounded-3xl overflow-hidden aspect-[21/9] border border-white/10 max-w-3xl relative"><Image src={about.teamPhoto || DEFAULT_ABOUT.teamPhoto} alt="The Good Secrets Safaris team" fill sizes="(min-width: 768px) 768px, 100vw" className="object-cover object-[center_45%]" /></div>
        <div className="grid sm:grid-cols-2 gap-6 max-w-3xl">{about.teamMembers.filter(member => member.active).map(({ id, name, role, bio, imageUrl }) => <article key={id} className="overflow-hidden rounded-3xl border border-white/10 bg-white/5"><div className="aspect-[4/3] bg-gradient-to-br from-action to-shell flex flex-col items-center justify-center text-center p-6 overflow-hidden relative">{imageUrl ? <Image src={imageUrl} alt={`${name}, ${role}`} fill sizes="(min-width: 640px) 384px, 100vw" className="object-cover" /> : <><Camera className="w-8 h-8 text-brand-soft" /><strong className="text-white mt-3">Photo coming soon</strong><span className="text-xs text-on-shell-subtle mt-1">Placeholder pending an approved portrait</span></>}</div><div className="p-6"><h3 className="font-serif-luxury text-xl font-bold text-white">{name}</h3><span className="text-xs font-bold uppercase tracking-wider text-brand-soft">{role}</span><p className="text-sm text-on-shell-muted mt-3 leading-relaxed">{bio}</p></div></article>)}</div>
