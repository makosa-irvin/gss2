...
export default function AboutPage() {
  return <div className="pb-20">
    <section className="relative min-h-[560px] overflow-hidden border-b border-white/10">
      <Image src="/images/catalog/vehicle-next-to-the-great-migration.jpg" alt="Safari vehicle beside the Great Migration in East Africa" fill priority className="object-cover" />
    ...
    <main className="max-w-7xl mx-auto px-4 sm:px-8 py-14 sm:py-20 space-y-20">
      ...
      <section className="space-y-8">
        ...
        <div className="rounded-3xl overflow-hidden aspect-[21/9] border border-white/10 max-w-3xl relative"><Image src="/images/catalog/team-at-nairobi-national-park.jpg" alt="The Good Secrets Safaris team at the Nairobi National Park main gate" fill className="object-cover object-[center_45%]" /></div>
        <div className="grid sm:grid-cols-2 gap-6 max-w-3xl">{namedTeam.map(({ name, role, bio, photo }) => <article key={name} className="overflow-hidden rounded-3xl border border-white/10 bg-white/5"><div className="aspect-[4/3] bg-gradient-to-br from-action to-shell flex flex-col items-center justify-center text-center p-6 overflow-hidden relative">{photo ? <Image src={photo} alt={`${name}, ${role}`} fill className="object-cover" /> : <><Camera className="w-8 h-8 text-brand-soft" /><strong className="text-white mt-3">Photo coming soon</strong><span className="text-xs text-on-shell-subtle mt-1">Placeholder pending an approved portrait</span></>}</div><div className="p-6"><h3 className="font-serif-luxury text-xl font-bold text-white">{name}</h3><span className="text-xs font-bold uppercase tracking-wider text-brand-soft">{role}</span><p className="text-sm text-on-shell-muted mt-3 leading-relaxed">{bio}</p></div></article>)}</div>
