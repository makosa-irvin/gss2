...
export default async function BlogDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <div className="max-w-4xl mx-auto px-4 sm:px-8 py-8 space-y-8">
    ...
    <header className="space-y-4"><div className="flex flex-wrap items-center gap-2 text-xs font-bold uppercase tracking-wider text-[#e6bc65]"><span>{post.category}</span><span aria-hidden="true">·</span><span className="text-[#c7d2cb]">{post.readingTime}</span></div><h1 className="font-serif-luxury text-3xl sm:text-5xl font-bold text-white leading-tight">{post.title}</h1>{post.author ? <div className="flex items-center gap-3 pt-1 text-sm text-[#c7d2cb]">{post.author.avatar ? <Image src={post.author.avatar} alt="" width={44} height={44} className="w-11 h-11 rounded-full object-cover border border-[#e6bc65]" /> : null}<div><span className="font-bold text-white block">{post.author.name}</span><span>{post.author.role} · Published {new Date(post.publishedDate).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' })}</span></div></div> : null}</header>
