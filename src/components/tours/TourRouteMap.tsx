...
export const TourRouteMap: React.FC<{ tour: Tour }> = ({ tour }) => {
  ...
  const routeImage = fileName ? `${baseUrl.replace(/\/$/, '')}/images/routes/${fileName}` : undefined;
  const correctedTitle = correctedTitles[tour.slug];

  if (routeImage && !imageFailed) {
    return (
      <figure className="overflow-hidden rounded-2xl border border-[#d6d1c5] bg-[#fffdf8] shadow-sm" aria-labelledby={`route-map-caption-${tour.id}`}>
        <div className="relative bg-[#eee9dc]">
          <img
            src={routeImage}
          ...
          />
          {correctedTitle && (
            <div className="pointer-events-none absolute left-0 top-0 max-w-[58%] rounded-br-[22px] bg-[#123b2a] px-3 py-2 sm:px-5 sm:py-3 text-white shadow-sm">
              <strong className="block font-serif-luxury text-[clamp(12px,2.2vw,24px)] leading-tight tracking-wide">{correctedTitle}</strong>
              <span className="mt-0.5 block text-[clamp(7px,1.15vw,12px)] font-bold tracking-[0.08em]">ROUTE MAP</span>
            </div>
          )}
        </div>
  ...
  return (
    <div className="rounded-2xl border border-dashed border-[#b9b1a2] bg-[#f7f3ea] px-5 py-6 text-sm text-[#536158]" role="status">
      {fileName ? 'The route map image could not be loaded. Please refresh the page or try again shortly.' : 'A dedicated route map has not been added for this safari yet.'}
    </div>
  );
};
