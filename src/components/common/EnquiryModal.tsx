...
export const EnquiryModal: React.FC<EnquiryModalProps> = ({ isOpen, onClose, selectedTour, selectedHotel, initialType, initialSpecialRequests }) => {
  ...
  useEffect(() => {
    ...
    setSubmitError('');
  }, [isOpen, initialSpecialRequests, selectedTour?.id, selectedHotel?.id, initialType]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-sm overflow-y-auto" onMouseDown={e => { if (e.target === e.currentTarget) handleClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="enquiry-title" aria-describedby="enquiry-description" className="relative w-full max-w-2xl rounded-3xl bg-white border border-[#ded8cb] shadow-2xl p-5 sm:p-8 my-4 sm:my-8 text-left">
        <button ref={closeButtonRef} type="button" onClick={handleClose} aria-label="Close enquiry form" className="absolute top-4 right-4 min-w-11 min-h-11 flex items-center justify-center rounded-full bg-[#f4f1e8] hover:bg-[#eae5d8] text-[#303e35] border border-[#d7d1c4] focus:outline-none focus:ring-2 focus:ring-[#8a611d]"><X className="w-5 h-5" /></button>
