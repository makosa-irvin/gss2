...
export const EnquiryModal: React.FC<EnquiryModalProps> = ({ isOpen, onClose, selectedTour, selectedHotel, initialType, initialDestination, initialSpecialRequests, contextItems = [] }) => {
  ...
  const inputClass = 'w-full min-h-11 px-3.5 py-2.5 rounded-xl bg-[#faf8f2] border border-[#d7d1c4] text-sm text-[#161f19] focus:border-[#8a611d] focus:ring-2 focus:ring-[#8a611d]/20 focus:outline-none';
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/70 backdrop-blur-sm overflow-hidden p-2 sm:p-4" onMouseDown={e => { if (e.target === e.currentTarget) handleClose(); }}>
      <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="enquiry-title" aria-describedby="enquiry-description" className="relative w-full max-w-2xl max-h-[calc(100dvh-1rem)] sm:max-h-[calc(100dvh-2rem)] overflow-y-auto overscroll-contain rounded-2xl sm:rounded-3xl bg-white border border-[#ded8cb] shadow-2xl p-5 sm:p-8 text-left">
        <button ref={closeButtonRef} type="button" onClick={handleClose} aria-label="Close enquiry form" className="sticky top-0 z-20 float-right -mr-1 -mt-1 min-w-11 min-h-11 flex items-center justify-center rounded-full bg-[#f4f1e8] hover:bg-[#eae5d8] text-[#303e35] border border-[#d7d1c4] shadow-sm focus:outline-none focus:ring-2 focus:ring-[#8a611d]"><X className="w-5 h-5" /></button>

        {!isSubmitted ? <div className="clear-both">
          <div className="mb-5 -mt-8 pr-12"><span className="text-xs font-extrabold uppercase tracking-wider text-[#76541a]">No-obligation enquiry</span><h2 id="enquiry-title" className="font-serif-luxury text-2xl sm:text-3xl font-bold text-[#161f19] mt-1 leading-tight">{title}</h2><p id="enquiry-description" className="text-sm text-[#46544b] mt-2 leading-relaxed">We already carry across the safari, destination or shortlist you were viewing. Add only the details needed to shape the right proposal.</p></div>

          {enquiryItems.length > 0 && <div className="mb-5 rounded-2xl border border-[#d8e5dc] bg-[#f2f8f4] p-4"><div className="flex items-center gap-2 text-xs font-extrabold uppercase tracking-wider text-[#1b4332]"><MapPin className="w-4 h-4" />Included in this enquiry</div><div className="mt-2 flex flex-wrap gap-2">{enquiryItems.map(item => <span key={item} className="rounded-full border border-[#c8ddcf] bg-white px-3 py-1.5 text-xs font-semibold text-[#303e35]">{item}</span>)}</div></div>}
          {hasRememberedDetails && <p className="mb-5 rounded-xl border border-[#e3ddcf] bg-[#faf8f2] px-3 py-2 text-xs text-[#536158]">We prefilled traveller details from an earlier successful enquiry in this browser session. You can edit them before sending.</p>}

          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4"><div><label htmlFor="enquiry-name" className={labelClass}>Full name *</label><input id="enquiry-name" autoComplete="name" required value={fullName} onChange={e => setFullName(e.target.value)} className={inputClass} /></div><div><label htmlFor="enquiry-email" className={labelClass}>Email *</label><input id="enquiry-email" type="email" autoComplete="email" required value={email} onChange={e => setEmail(e.target.value)} className={inputClass} /></div><div><label htmlFor="enquiry-phone" className={labelClass}>Phone / WhatsApp <span className="font-normal text-[#66766b]">(optional)</span></label><input id="enquiry-phone" type="tel" autoComplete="tel" value={phone} onChange={e => setPhone(e.target.value)} className={inputClass} /></div><div><label htmlFor="enquiry-country" className={labelClass}>Country <span className="font-normal text-[#66766b]">(optional)</span></label><input id="enquiry-country" autoComplete="country-name" value={country} onChange={e => setCountry(e.target.value)} className={inputClass} /></div></div>
            ...
            <div><label htmlFor="enquiry-notes" className={labelClass}>Anything else we should know? <span className="font-normal text-[#66766b]">(optional)</span></label><textarea id="enquiry-notes" rows={3} value={specialRequests} onChange={e => setSpecialRequests(e.target.value)} placeholder="Celebration, accessibility, dietary needs, wildlife priorities, preferred pace..." className={`${inputClass} min-h-24`} /></div>
            ...
            <div className="rounded-xl bg-[#f6f3ea] border border-[#e3ddcf] p-3 flex gap-2 text-xs text-[#46544b]"><ShieldCheck className="w-4 h-4 text-[#76541a] shrink-0 mt-0.5" /><span>Submitting is an enquiry only. No payment is taken and no booking is confirmed. Contact details are remembered only for this browser session after a successful enquiry.</span></div>
            <div className="flex flex-col-reverse sm:flex-row sm:items-center sm:justify-between gap-3 pb-1"><a href={getWhatsAppUrl({ tourTitle: selectedTour?.title, hotelTitle: selectedHotel?.name })} target="_blank" rel="noopener noreferrer" className="min-h-12 inline-flex items-center justify-center gap-2 px-5 rounded-xl bg-[#eef7f2] hover:bg-[#def0e6] text-[#1b4332] text-sm font-bold border border-[#b9d8c5]"><MessageCircle className="w-4 h-4" />Ask on WhatsApp</a><button type="submit" disabled={isSubmitting} className="min-h-12 inline-flex items-center justify-center gap-2 px-7 rounded-xl bg-[#8a611d] hover:bg-[#704d15] disabled:opacity-60 disabled:cursor-wait text-white font-extrabold text-sm shadow-md"><span>{isSubmitting ? 'Sending…' : 'Request my safari quote'}</span><Send className="w-4 h-4" /></button></div>
          </form>
        </div> : <div className="clear-both text-center py-10 space-y-4"><div className="w-16 h-16 rounded-full bg-[#1b4332] text-white flex items-center justify-center mx-auto ring-8 ring-[#1b4332]/15"><CheckCircle2 className="w-8 h-8" /></div><h2 id="enquiry-title" className="font-serif-luxury text-3xl font-bold text-[#161f19]">Thanks — we received your enquiry.</h2><p id="enquiry-description" className="text-sm text-[#46544b] max-w-md mx-auto leading-relaxed">Your safari context and traveller details were sent together, so you will not need to repeat the same information in a follow-up enquiry during this browser session.</p><div className="flex justify-center"><button onClick={handleClose} className="min-h-11 px-6 rounded-xl bg-[#8a611d] text-white font-bold text-sm hover:bg-[#704d15] inline-flex items-center gap-2"><Sparkles className="w-4 h-4" />Continue planning</button></div></div>}
      </div>
    </div>
  );
};
