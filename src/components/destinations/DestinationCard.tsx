      ...
      <button type="button" onClick={() => onSelect(destination)} aria-label={`Explore ${destination.name}, ${destination.country}`} className="block w-full text-left rounded-2xl">
        <div className="relative aspect-[4/3] sm:aspect-[16/11] w-full overflow-hidden bg-[#17231b]">
          ...
          <div className="absolute inset-x-0 bottom-0 p-5 flex flex-col justify-end">
            <div className="flex items-center gap-1.5 text-xs text-[#f0cb7a] font-bold mb-1"><Compass className="w-3.5 h-3.5" aria-hidden="true" /><span>{destination.recommendedDuration || '2 - 4 Days'}</span></div>
            <h3 className="font-serif-luxury text-xl font-bold text-white group-hover:text-[#f0cb7a] transition-colors leading-snug">{destination.name}</h3>
            ...
            {destination.wildlife && destination.wildlife.length > 0 && <div className="mt-3 flex flex-wrap gap-1.5">{destination.wildlife.slice(0, 3).map(animal => <span key={animal} className="rounded-md bg-black/70 backdrop-blur-sm px-2 py-1 text-xs font-medium text-white border border-white/25">{animal}</span>)}{destination.wildlife.length > 3 && <span className="text-xs text-[#f0cb7a] font-bold self-center ml-1">+{destination.wildlife.length - 3} more</span>}</div>}
            <div className="mt-4 flex items-center justify-between pt-3 border-t border-white/30"><span className="text-sm font-bold text-[#f0cb7a] group-hover:underline flex items-center gap-1">Explore destination <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" aria-hidden="true" /></span></div>
          </div>
        </div>
      </button>
    </article>
