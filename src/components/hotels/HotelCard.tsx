...
import { MapPin, Star, Check, MessageCircle, ArrowRight } from 'lucide-react';
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[16/10] w-full overflow-hidden bg-[#f4f2ec]">
        ...
        <div className="absolute top-3 left-3 right-3 flex items-center justify-between pointer-events-none">
          <div className="flex flex-wrap gap-1.5">
            ...
            <span className="rounded-full bg-black/70 backdrop-blur-md text-white font-semibold px-2.5 py-1 text-xs border border-white/30">
              {hotel.category}
            </span>
          </div>
      ...
      <div className="flex flex-1 flex-col p-5 justify-between bg-white">
        <div>
          <div className="flex items-center gap-1 text-xs font-bold text-[#76541a] mb-1.5">
          ...
          <div className="mt-3 flex flex-wrap gap-1.5">
            {hotel.isFamilyFriendly && (
              <span className="rounded-md bg-[#eef7f2] px-2 py-0.5 text-xs font-semibold text-[#1b4332] border border-[#cbe4d4]">
                Family Friendly
              </span>
            )}
            {hotel.isHoneymoonFriendly && (
              <span className="rounded-md bg-[#fdf2f8] px-2 py-0.5 text-xs font-semibold text-[#9d174d] border border-[#f3bdd8]">
                Honeymoon
              </span>
            )}
            {hotel.isSeniorFriendly && (
              <span className="rounded-md bg-[#eff6ff] px-2 py-0.5 text-xs font-semibold text-[#1e40af] border border-[#bfdbfe]">
                Senior Friendly
              </span>
            )}
          </div>
          ...
          {hotel.facilities && (
            <div className="mt-3 space-y-1.5">
              {hotel.facilities.slice(0, 2).map((fac, idx) => (
                <div key={idx} className="flex items-center gap-1.5 text-xs text-[#425046]">
                  <Check className="w-3.5 h-3.5 text-[#76541a] shrink-0" aria-hidden="true" />
        ...
        <div className="mt-5 pt-4 border-t border-[#e4dfd4] flex items-end justify-between gap-3">
          <div>
            ...
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-xs text-[#58675d]">/ night</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            <a
              ...
              aria-label={`Enquire on WhatsApp about ${hotel.name}`}
