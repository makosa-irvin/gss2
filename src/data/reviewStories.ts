...
export type ReviewStory = {
  platform: 'Tripadvisor' | 'SafariBookings';
  reviewer: string;
  title: string;
  date: string;
  rating: number;
...
/**
 * Destination tags are added only when the independent source explicitly names
 * that place in the review text. They let tour pages surface relevant traveller
 * stories without implying the reviewer travelled the exact itinerary shown.
 */
...
const aliases: Record<string, string[]> = {
  'maasai mara': ['maasai mara', 'masai mara', 'mara'],
  'lake nakuru': ['lake nakuru', 'nakuru'],
  'lake naivasha': ['lake naivasha', 'naivasha'],
  'amboseli': ['amboseli'],
  'ol pejeta': ['ol pejeta'],
  'nairobi national park': ['nairobi national park'],
  'hell’s gate': ["hell's gate", 'hell’s gate', 'hells gate'],
  'mount longonot': ['mount longonot', 'mt longonot', 'longonot'],
  'crescent island': ['crescent island'],
};

const normalize = (value: string) => value.toLowerCase().replace(/[’']/g, "'").trim();

const placeMatches = (tourPlace: string, reviewPlace: string) => {
  const a = normalize(tourPlace);
  const b = normalize(reviewPlace);
  const candidates = aliases[b] ?? [b];
  return candidates.some(candidate => a.includes(candidate) || candidate.includes(a));
};

export function getRouteReviewStories(tourDestinations: string[] = [], limit = 3): ReviewStory[] {
  if (!tourDestinations.length) return [];
  return featuredReviews
    .filter(review => review.destinations.some(reviewPlace => tourDestinations.some(tourPlace => placeMatches(tourPlace, reviewPlace))))
    .slice(0, limit);
}
