...
export const TourDetailView: React.FC<TourDetailViewProps> = ({ tour, onBack, onOpenEnquiryModal }) => {
  ...
  const routeMapImage = tour.images?.find(image => /map|route/i.test(image));
  ...
  return (
    <div className="pb-28 lg:pb-12">
      ...
      <section className="relative min-h-[500px] sm:min-h-[570px] overflow-hidden bg-[#0c120e]">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-8 pt-6 pb-10 min-h-[500px] sm:min-h-[570px] flex flex-col justify-between">
          ...
          <div className="max-w-3xl">
            {(tour.featured || tour.popular) && <span className="inline-flex rounded-full bg-[#1b4332] px-3 py-1.5 text-xs font-bold text-white shadow-sm mb-4">{tour.popular ? 'Popular Choice' : 'Featured Safari'}</span>}
      ...
      <div className="sticky top-0 z-40 bg-[#fffdf8]/98 backdrop-blur-xl border-b border-[#ddd7ca] shadow-md">
        <div className="max-w-7xl mx-auto lg:flex lg:items-stretch">
          <nav aria-label="Tour sections" className="min-w-0 flex-1 px-2 sm:px-8 flex overflow-x-auto scrollbar-hide">
          ...
          <div className="hidden lg:flex shrink-0 items-center gap-4 border-l border-[#ddd7ca] px-5 bg-white">
            <div><span className="block text-xs font-semibold text-[#76541a]">From</span><strong className="font-serif-luxury text-xl text-[#161f19]">{price}</strong><span className="block text-[11px] text-[#536158]">Per person sharing</span></div>
            <button type="button" onClick={() => onOpenEnquiryModal({ selectedTour: tour })} className="min-h-11 rounded-xl bg-[#1b4332] hover:bg-[#123524] px-5 text-sm font-extrabold text-white">Get Quote</button>
          </div>
        </div>
      </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-8 py-8 sm:py-12 space-y-14">
        <section id="overview" className="scroll-mt-24 grid lg:grid-cols-12 gap-8 lg:gap-12 items-start">
          <div className="lg:col-span-5 space-y-5"><div><span className="text-xs font-bold uppercase tracking-wider text-[#e6bc65]">The experience</span><h2 className="font-serif-luxury text-3xl font-bold text-white mt-1">Safari overview</h2></div><p className="text-base text-[#c7d2cb] leading-relaxed whitespace-pre-line">{tour.fullDescription || tour.shortDescription}</p><div className="grid grid-cols-2 gap-3 pt-2"><div className="rounded-xl border border-white/10 bg-white/5 p-3"><Clock className="w-5 h-5 text-[#e6bc65] mb-2" /><span className="text-xs text-[#aebbb2] block">Duration</span><strong className="text-sm text-white">{tour.durationLabel}</strong></div><div className="rounded-xl border border-white/10 bg-white/5 p-3"><Compass className="w-5 h-5 text-[#e6bc65] mb-2" /><span className="text-xs text-[#aebbb2] block">Travel style</span><strong className="text-sm text-white">{tour.travelStyles?.[0] || 'Tailor-made'}</strong></div></div></div>
          <div id="gallery" className="scroll-mt-24 lg:col-span-7 space-y-3"><div className="aspect-[4/3] sm:aspect-[16/10] rounded-2xl overflow-hidden relative bg-[#f4f2ec] border border-white/10"><img src={tour.images?.[activeImageIdx] || tour.images?.[0]} alt={`${tour.title} gallery image ${activeImageIdx + 1}`} className="w-full h-full object-cover" /><span className="absolute bottom-3 right-3 rounded-full bg-black/70 px-3 py-1.5 text-xs font-semibold text-white">{activeImageIdx + 1} / {tour.images?.length || 1}</span></div>{tour.images?.length > 1 && <div className="flex gap-2 overflow-x-auto pb-1">{tour.images.map((img, idx) => <button type="button" key={img + idx} onClick={() => setActiveImageIdx(idx)} aria-pressed={activeImageIdx === idx} aria-label={`View photo ${idx + 1}`} className={`shrink-0 w-20 h-16 sm:w-28 sm:h-20 rounded-xl overflow-hidden border-2 ${activeImageIdx === idx ? 'border-[#e6bc65]' : 'border-transparent opacity-75'}`}><img src={img} alt="" className="w-full h-full object-cover" /></button>)}</div>}</div>
        </section>
        ...
        <section id="route" className="scroll-mt-24 space-y-5">
          <div><span className="text-xs font-bold uppercase tracking-wider text-[#e6bc65]">Your route</span><h2 className="font-serif-luxury text-2xl sm:text-3xl font-bold text-white mt-1">The journey on the map</h2><p className="text-sm text-[#c7d2cb] mt-2">A simple route image gives geographic context without loading an interactive map library.</p></div>
          {routeMapImage ? <div className="overflow-hidden rounded-2xl border border-white/10 bg-white"><img src={routeMapImage} alt={`Map showing the route for ${tour.title}`} className="w-full aspect-[16/6] sm:aspect-[16/5] object-cover" loading="lazy" /></div> : <div className="rounded-2xl border border-[#ded8cb] bg-[#f5f0e4] p-6 sm:p-8"><div className="flex flex-wrap items-center gap-3">{routeStops.map((stop, idx) => <React.Fragment key={`${stop}-${idx}`}><span className="inline-flex min-h-10 items-center rounded-full bg-[#1b4332] px-4 text-sm font-bold text-white">{idx + 1}. {stop}</span>{idx < routeStops.length - 1 && <span aria-hidden="true" className="text-[#76541a] font-bold">→</span>}</React.Fragment>)}</div><p className="mt-4 text-sm text-[#536158]">Add a static route-map image to this tour's image data to replace this fallback automatically.</p></div>}
        </section>

        <section id="inclusions" className="scroll-mt-24 grid sm:grid-cols-2 gap-5">{includedItems.length > 0 && <div className="rounded-2xl bg-emerald-50 border border-emerald-200 p-5 sm:p-6"><h2 className="font-serif-luxury text-xl font-bold text-[#163524]">Included in your safari</h2><ul className="mt-4 space-y-2.5">{includedItems.map((item, idx) => <li key={idx} className="flex gap-2 text-sm text-[#254331]"><Check className="w-4 h-4 mt-0.5 shrink-0" /><span>{item}</span></li>)}</ul></div>}{excludedItems.length > 0 && <div className="rounded-2xl bg-rose-50 border border-rose-200 p-5 sm:p-6"><h2 className="font-serif-luxury text-xl font-bold text-rose-900">Not included</h2><ul className="mt-4 space-y-2.5">{excludedItems.map((item, idx) => <li key={idx} className="flex gap-2 text-sm text-rose-900"><X className="w-4 h-4 mt-0.5 shrink-0" /><span>{item}</span></li>)}</ul></div>}</section>
        <section id="pricing" className="scroll-mt-24"><DynamicPricingTable tour={tour} /></section>
        <section id="faqs" className="scroll-mt-24 rounded-2xl bg-white border border-[#ded8cb] p-5 sm:p-7"><h2 className="font-serif-luxury text-2xl font-bold text-[#161f19]">Safari essentials</h2><div className="mt-5 grid md:grid-cols-2 gap-3 text-sm text-[#405046]"><div className="rounded-xl bg-[#faf8f2] border border-[#e3ddcf] p-4"><strong className="block text-[#161f19] mb-1">What should I pack?</strong>Light neutral clothing, a warm layer for dawn drives, sun protection, binoculars and a soft-sided travel bag are practical starting points.</div><div className="rounded-xl bg-[#faf8f2] border border-[#e3ddcf] p-4"><strong className="block text-[#161f19] mb-1">Can this itinerary change?</strong>Yes. This safari is a starting point and can be adapted around your dates, pace, interests and accommodation preferences.</div></div></section>
      ...
      <div className="lg:hidden fixed inset-x-0 bottom-0 z-50 border-t border-[#d7d1c4] bg-[#fffdf8]/98 backdrop-blur-xl px-4 pt-3 pb-[max(12px,env(safe-area-inset-bottom))] shadow-[0_-8px_30px_rgba(0,0,0,0.16)]"><div className="max-w-lg mx-auto flex items-center gap-4"><div className="min-w-0 flex-1"><span className="block text-xs font-semibold text-[#76541a]">From</span><strong className="font-serif-luxury text-2xl leading-none text-[#161f19]">{price}</strong><span className="block text-[11px] text-[#536158] mt-1">Per person sharing</span></div><button type="button" onClick={() => onOpenEnquiryModal({ selectedTour: tour })} className="min-h-12 min-w-[150px] rounded-xl bg-[#1b4332] hover:bg-[#123524] px-5 text-sm font-extrabold text-white">Get Quote</button></div>{isKenyanResidentMode && tour.residentPriceKES && <p className="max-w-lg mx-auto mt-1 text-xs font-semibold text-[#1b4332]">Resident rate: KSH {tour.residentPriceKES.toLocaleString()}</p>}</div>
    </div>
  );
};
