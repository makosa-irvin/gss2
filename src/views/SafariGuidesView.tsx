...
export const SafariGuidesView: React.FC = () => {
  ...
  if (slug && guide) return (
    <div className="max-w-5xl mx-auto px-4 sm:px-8 py-10 sm:py-14 space-y-8">
      ...
      <article className="rounded-3xl bg-white border border-border-strong p-7 sm:p-10 text-ink space-y-9">{guide.sections.map(section => <section key={section.heading}><h2 className="font-serif-luxury text-3xl font-bold text-ink-strong">{section.heading}</h2><p className="mt-3 text-base leading-relaxed">{section.body}</p></section>)}<div className="rounded-2xl bg-surface-soft border border-border-strong p-5"><strong className="text-ink-strong">Use this as planning guidance, not a fixed rule.</strong><p className="text-sm mt-1">Seasonal conditions, park rules, fees and availability change. A dated itinerary should confirm the details that matter for your trip.</p></div></article>
